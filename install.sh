...
# Vérifier/Installer Certbot pour les certificats Let's Encrypt
echo "Vérification de Certbot..."
if command -v certbot &> /dev/null; then
    CERTBOT_VERSION=$(certbot --version | awk '{print $2}')
    echo -e "${GREEN}✔ Certbot ${CERTBOT_VERSION} installé${NC}"
else
    echo -e "${YELLOW}⚠ Certbot non détecté, installation...${NC}"
